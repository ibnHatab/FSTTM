...
func init() {
	typemap.RegisterMessage("unitree_go/PathPoint", PathPointTypeSupport)
	typemap.RegisterMessage("unitree_go/msg/PathPoint", PathPointTypeSupport)
}
...
func (t *PathPoint) CloneMsg() types.Message {
	return t.Clone()
}
...
func (t *PathPoint) GetTypeSupport() types.MessageTypeSupport {
	return PathPointTypeSupport
}

// PathPointPublisher wraps rclgo.Publisher to provide type safe helper
// functions
...
// PathPoint
func NewPathPointPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*PathPointPublisher, error) {
	pub, err := node.NewPublisher(topic_name, PathPointTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// PathPointSubscription wraps rclgo.Subscription to provide type safe helper
// functions
...
type PathPointSubscriptionCallback func(msg *PathPoint, info *rclgo.MessageInfo, err error)

// NewPathPointSubscription creates and returns a new subscription for the
// PathPoint
func NewPathPointSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback PathPointSubscriptionCallback) (*PathPointSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg PathPoint
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, PathPointTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (s *PathPointSubscription) TakeMessage(out *PathPoint) (*rclgo.MessageInfo, error) {
...
var PathPointTypeSupport types.MessageTypeSupport = _PathPointTypeSupport{}

type _PathPointTypeSupport struct{}

func (t _PathPointTypeSupport) New() types.Message {
	return NewPathPoint()
}
...
func (t _PathPointTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	m := msg.(*PathPoint)
...
func (t _PathPointTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	m := msg.(*PathPoint)
...
type CPathPoint__Sequence = C.unitree_go__msg__PathPoint__Sequence

func PathPoint__Sequence_to_Go(goSlice *[]PathPoint, cSlice CPathPoint__Sequence) {
	if cSlice.size == 0 {
		return
	}
...
func PathPoint__Sequence_to_C(cSlice *CPathPoint__Sequence, goSlice []PathPoint) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PathPoint__Array_to_Go(goSlice []PathPoint, cSlice []CPathPoint) {
	for i := 0; i < len(cSlice); i++ {
		PathPointTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}
func PathPoint__Array_to_C(cSlice []CPathPoint, goSlice []PathPoint) {
